...
class CustomInputField extends StatelessWidget {
  ...
  // Determinamos el color según las validaciones
  if (isRequired && value.isEmpty) {
    return Colors.red; // Error: campo vacío
  } else if (isNumeric && numValue == null) {
    return Colors.red; // Error: no es un número válido
  } else if (isNumeric && ((min != null && numValue! < min!) || (max != null && numValue! > max!))) {
    return  Color.fromARGB(255, 214, 162, 6); // Advertencia: fuera del rango
  } else {
    return Colors.grey.shade500;// Correcto: valor válido
  }
}
    return Padding(
 ...
 Widget _buildText() {
  return Builder(
    builder: (context) {
      ...
      if (isNumeric && numValue != null && ((min != null && numValue < min!) || (max != null && numValue > max!))) {
        return Padding(
          padding: const EdgeInsets.only(top: 5.0, left: 20.0,bottom: 3),
          child: Text(
            'Debe estar entre $min y $max',
            style: const TextStyle(fontSize: 13, height: 1, color: Color.fromARGB(255, 214, 162, 6)),
          ),
        );
      } else {
        return const SizedBox(); // Devolvemos un widget vacío si no hay error
      }
    },
  );
} 
